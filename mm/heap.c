 ...
 *       Filename:  heap.c
 *
 *    Description:  堆内存管理的实现
 *
 *        Version:  1.0
 *        Created:  2015年04月20日 10时05分02秒
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  Morain
...
{
	//切分内存块之前需要保证之后的剩余内存至少容纳一个内存管理块的大小
	if(chunk->length - len > sizeof(header_t)){
		header_t *newchunk = (header_t *)((uint32_t)chunk + len /*chunk->length*/);
...
void test_heap()
{
	printk_color(rc_black , rc_magenta , "Test kmalloc & kfree...\n\n");
